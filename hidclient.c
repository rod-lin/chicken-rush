...
int	initevents(unsigned int evdevmask, int mutex11)
{
    ...
    for (i = j = 0; j < MAXEVDEVS; ++j) {
        if (evdevmask != 0 && (evdevmask & (1 << j)) == 0)
            continue;
        ...
        if (0 <= eventdevs[i]) {
            ...
            if (mutex11 > 0 && xinlist != NULL) {
                ...
                if (ioctl(eventdevs[i], EVIOCGNAME(256), xinlist + 3801) >= 0) {
                    ...
                    while (*p != 0 && NULL != (p = strstr(p, xinlist + 3801))) {
...
void closeevents(void)
{
    ...
    for (i = 0; i < MAXEVDEVS; ++i) {
        if (eventdevs[i] >= 0) {
            close(eventdevs[i]);
            if (x11handles[i] >= 0) {
                sprintf(buf, "xinput set-int-prop %d \"Device Enabled\" 8 1", x11handles[i]);
                if (system(buf)) {
...
int	list_input_devices(void)
{
    ...
    pf = popen("xinput --list --name-only", "r");
...
int	parse_events(fd_set * efds, int sockdesc)
{
    ...
    for (i = 0; i < MAXEVDEVS; ++i) {
        ...
        if (-1 == j) {
            if (debugevents & 0x1) {
                if (errno > 0) {
                    fprintf(stderr, "%d|%d(%s) (expected %d bytes). ", eventdevs[i],
                            errno, strerror(errno), (int)sizeof(struct input_event));
                } else {
        ...
        if (debugevents & 0x1)
            fprintf(stdout, "EVENT{%04X %04X %08X}\n", inevent->type, inevent->code, inevent->value);
        ...
        switch (inevent->type) {
            ...
            case EV_KEY:
                ...
                switch (inevent->code)
                {
                    ...
                    case KEY_PAGEUP: ++u;
                    case KEY_HOME: ++u;
                    case KEY_INSERT: ++u; ++u; //[Pause] key // - checked separately
            ...
            case EV_REL:
                switch (inevent->code) {
                    ...
                    case REL_WHEEL:
                        ...
                        evmouse->rep_id = REPORTID_MOUSE;
                        evmouse->button = mousebuttons & 0x07;
                        evmouse->axis_x = (inevent->code == ABS_X ? inevent->value : 0);
                        evmouse->axis_y = (inevent->code == ABS_Y ? inevent->value : 0);
                        evmouse->axis_z = (inevent->code >= ABS_Z ? inevent->value : 0);
...
int	main(int argc, char ** argv)
{
    ...
    while (0 == prepareshutdown) {
        ...
        usleep(500000);
        // Sleep 0.5 secs between connections
        // to not be flooded
    }
